<template>
  <NavVar />
  <div id="house-form" v-if="!id || (id && !isFetchingHousePhoto)">
    <div v-if="!id" class="top-row">
    ...
    <form @submit.prevent="formSubmit" novalidate>
      ...
      <div>
        ...
        <div id="house-photo-input-container">
          ...
          <img class="house-photo" :src="houseImage" alt="house photo" v-if="!fileName && id && houseImage">
          <img :src="url" v-if="fileName" class="house-photo" alt="house photo">
        </div>
      </div>
      ...
      <div id="post-button-container">
        ...
        <button type="submit" v-if="!id" :disabled="buttonDisabled">POST</button>
        <button type="submit" v-if="id" :disabled="buttonDisabled">SAVE</button>
      </div>
    </form>
  </div>
</template>
...
let houseImage = ""
// to combat a bug where house photo was not shown in some random cases
const isFetchingHousePhoto = ref(true)
...
onBeforeMount(async () => {
  if (props.id) {
    const house = await houseStore.getOneHouse(props.id)
    ...
    isFetchingHousePhoto.value = false
  }
})
...
function validateInputs() {
  if (checkRequiredFields() && checkNumberInputs()) {
    // check if zipcode is valid 
    const regex = new RegExp('^[0-9]{4}[A-Z]{2}$')
    if (!regex.test(zip.value.replace(" ", "").toUpperCase())) hadleBadInput("postal-code-input", "invalid postal code")
    else return true
  }
}
...
function checkRequiredFields() {
  ...
  else if (!fileName.value && !props.id) hadleBadInput("picture-input", "Required field missing")
...
function checkNumberInputs() {
  // check if the number inputs can be parsed as numbers
...
// defines the behaviour when the POST/EDIT button is pressed
const formSubmit = async function (e) {
  ...
  if (validateInputs()) {
    ...
    if (!props.id) {
      const isSuccessful = await houseStore.addHouse(body, fileName)
      ...
      await houseStore.updateHouse(props.id, body, fileName.value)
      buttonDisabled.value = false
    }
  } else {
    buttonDisabled.value = false
...
#house-form button[disabled=disabled],
button:disabled {
